/* Codex Research — capa de fluido del hero.
   Motor de fluidos WebGL de Pavel Dobryakov (MIT), reajustado a la paleta CLARA.

   Cómo funciona sobre fondo blanco: el simulador es ADITIVO — la tinta suma
   brillo sobre una base oscura, así que sobre #fafafa sería invisible. El motor
   sigue pintando claro-sobre-oscuro y es el CSS el que lo da vuelta:
     filter: invert(1)        -> el casi-negro pasa a casi-blanco; la tinta brillante, a oscura
     mix-blend-mode: multiply -> ese casi-blanco no altera la página; sólo mancha la tinta
   Por eso los colores de abajo son los COMPLEMENTOS de los de la marca: al
   invertirse caen en el gris pizarra de --accent (#525863).

   Se desactiva solo si el visitante pidió menos animación o no hay WebGL. */
function fluidSimulation(canvas) {
    canvas.width = canvas.clientWidth;
    canvas.height = canvas.clientHeight;

    let config = {
        SIM_RESOLUTION: 128,    // es un fondo sutil, no el protagonista
        DYE_RESOLUTION: 512,
        DENSITY_DISSIPATION: 0.965,
        VELOCITY_DISSIPATION: 0.96,
        PRESSURE_DISSIPATION: 0.8,
        PRESSURE_ITERATIONS: 20,
        CURL: 26,               // menos remolino: veteado tranquilo
        SPLAT_RADIUS: 0.30,     // manchas más anchas y difusas
        SHADING: true,
        COLORFUL: true,
        PAUSED: false,
    ...
    initFramebuffers();
    multipleSplats(18);
    for (let i = 0; i < 4; i++) splatStack.push(6 + parseInt(Math.random() * 6));
    ...
    function driveVirtualPointer() {
        ...
        if (!virtualColor || Date.now() - lastVColorTime > 120) {
            ...
            virtualColor.r *= 1.0; virtualColor.g *= 1.0; virtualColor.b *= 1.0;
    ...
    function generateColor() {
        // Banda ámbar/arena muy desaturada. Invertida por el CSS cae entre el
        // gris pizarra (#525863) y un azul frío apagado: la gama de Codex.
        const h = 0.06 + Math.random() * 0.10;
        let c = HSVtoRGB(h, 0.22, 1.0);
        c.r *= 0.26; c.g *= 0.26; c.b *= 0.26;
        return c;
    }
...
/* Arranque: sólo si hay canvas, WebGL y el visitante no pidió menos animación. */
(function () {
  var canvas = document.querySelector('.hero-fluid');
  if (!canvas) return;
  ...
  try {
    ...
    canvas.classList.add('is-on');   // se funde sólo cuando ya está pintando
  } catch (e) {
    canvas.remove();                 // sin WebGL, el hero se queda como estaba
  }
})();
